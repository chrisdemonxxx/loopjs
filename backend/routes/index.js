...
router.post('/login', authRateLimit, async (req, res) => {
  ...
  try {
    ...
    const dbConnected = sequelize && await sequelize.authenticate().then(() => true).catch(() => false);
    ...
    const accessToken = jwt.sign({ id: user.id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_ACCESS_TOKEN_EXPIRATION });
    const refreshToken = jwt.sign({ id: user.id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_REFRESH_TOKEN_EXPIRATION });
