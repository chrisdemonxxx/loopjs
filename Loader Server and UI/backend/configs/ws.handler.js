...
module.exports = (ws, req) => {
        ...
        try {
            ...
            if ((data.type === 'register' || data.type === 'agent_register') && clientType === 'stealth') {
                ...
                try {
                    ...
                    console.log('Integration layer not available, using basic registration');
                }
                return;
            }
    ...
    ws.on('close', async () => {
        ...
        if (ws.clientType === 'web') {
            ...
            // Send Telegram notification for disconnection
            if (updatedClient) {
                try {
                    await telegramService.notifyDisconnection(updatedClient);
                } catch (telegramError) {
                    console.error('Failed to send Telegram notification:', telegramError);
                }
