...
import { wsIntegration, apiIntegration } from './utils/integration';
...
export default function App() {
  ...
  const initWebSocket = () => {
    ...
    const handleMessage = (data: any) => {
      wsIntegration.handleMessage(data, {
        ...
        onTaskUpdate: (taskData: any) => {
          if (taskData.type === 'task_executed') {
            toast.success(`Task executed on client ${taskData.clientUuid}`);
  ...
  const handleProcess = async (agent: Agent, commandKey: string) => {
    try {
      await agentService.sendCommand(agent.id, commandKey);
      toast.success(`Task sent to ${agent.name}`);
