  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const commandsResponse = await fetch(`/api/commands/available/${selectedUser.uuid}`);
      ...
      <div className="bg-gray-800 px-6 py-4 border-b border-gray-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <span className="text-2xl">{getPlatformIcon(selectedUser.operatingSystem)}</span>
            <div>
              <h3 className="text-lg font-semibold text-white">Command Interface</h3>
              <p className="text-sm text-gray-400">
                {selectedUser.computerName} • {selectedUser.operatingSystem} {selectedUser.osVersion}
              </p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <div>
                  ...
                  <div className="text-purple-400 font-medium">
                    {Object.values(selectedUser.capabilities || {}).flat().length} available
                  </div>
                </div>
                <div>
